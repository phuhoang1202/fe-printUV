      ...
      <div className='flex lg:flex-row flex-col items-center gap-4 justify-center'>
        {dataCategory.map((category, index) => {
          return (
            <div
              key={index}
              className='flex flex-col items-center justify-center cursor-pointer shadow-md gap-4 border rounded-lg px-4 h-auto py-4 w-[320px] bg-white hover:shadow-lg'
            >
              <img src={category.image} alt='image' className='w-20 h-20 object-contain' />
              <h4 className='font-semibold text-lg uppercase'>{category.text}</h4>
            </div>
          )
        })}
      </div>
