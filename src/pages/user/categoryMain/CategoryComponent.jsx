...
export default function CategoryComponent() {
  ...
  useEffect(() => {
    if (location.state?.activeIndex !== undefined && location.state?.idCategory !== undefined) {
      setActiveIndex(location.state.activeIndex)
      setSelectedChildId(location.state.idCategory)
      setIsReady(true) // Đánh dấu đã sẵn sàng gọi API
    } else {
      setIsReady(true) // Trường hợp không có state từ router
    }
  }, [location.state])
  ...
  }, [isReady, selectedChildId, currentPage, optionSelect, optionSort])
  ...
  return (
    <div className='mx-auto max-w-7xl lg:mt-24 mt-28'>
      {/* Category Parent */}
      <div className='lg:px-0 px-2'>
        {dataCategory && (
          <div className='flex flex-nowrap items-center overflow-x-auto overflow-y-hidden pb-2 custom-scrollbar'>
            {/* Các danh mục khác */}
            {dataCategory.map((categoryParent, index) => {
              return (
                <div style={{ whiteSpace: 'nowrap' }}>
                  <button
                    key={index}
                    className={`px-2 min-w-20 flex justify-center items-center lg:h-10 h-9 transition-all duration-100 rounded-full capitalize ${
      ...
      <div className='lg:mt-8 mt-4'>
        <div className='flex items-center justify-between overflow-x-auto overflow-y-hidden lg:px-0 px-2 lg:pb-0 pb-2 custom-scrollbar'>
          <div className='flex items-center gap-[2px]'>
            ...
            {dataCategory[activeIndex]?.categories.map((childCategory) => (
              <button
                ...
                className={`min-w-10 px-4 lg:h-10 h-9 font-medium rounded-full transition-all duration-75  ${
